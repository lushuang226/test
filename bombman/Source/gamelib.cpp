...
void CAnimation::SetTopLeft(float nx, float ny)
{
	x = nx, y = ny;
...
void CAnimation::OnShow(double beishu)
{
	GAME_ASSERT(bmp.size() != 0, "CAnimation: Bitmaps must be loaded before they are shown.");
	bmp_iter->SetTopLeft(x, y);
	bmp_iter->ShowBitmap(beishu);
}

float CAnimation::Top()
{
	GAME_ASSERT(bmp.size() != 0,"CAnimation: Bitmaps must be loaded first.");
	return y;
}

float CAnimation::Width()
{
	GAME_ASSERT(bmp.size() != 0,"CAnimation: Bitmaps must be loaded first.");
...
void CMovingBitmap::SetTopLeft(float x, float y)
{
	GAME_ASSERT(isBitmapLoaded,"A bitmap must be loaded before SetTopLeft() is called !!!");
	float dx = location.left - x;
	float dy = location.top - y;
	location.left = x;
	location.top = y;
	location.right -= dx;
	location.bottom -= dy;
