...
public:
	CMovingBitmap();
	float   Height();						// 取得圖形的高度
	float   Left();						// 取得圖形的左上角的 x 座標
	...
	void  SetTopLeft(float, float);			// 將圖的左上角座標移至 (x,y)
...
public:
	...
	float   Height();					// 取得動畫的高度
	bool  IsFinalBitmap();			// 回傳正在撥放的bitmap是否為最後一個bitmap
	float   Left();					// 取得動畫的左上角的 x 座標
	void  OnMove();					// 依頻率更換bitmap
	void  OnShow();					// 將動畫貼到螢幕
	void  OnShow(double beishu);
	void  Reset();					// 重設播放順序回到第一張圖形
	void  SetDelayCount(int);		// 設定動畫播放速度的常數(越大越慢)
	void  SetTopLeft(float, float);		// 將動畫的左上角座標移至 (x,y)
	float   Top();					// 取得動畫的左上角的 y 座標
	float   Width();					// 取得動畫的寬度
private:
...
class CInteger {
public:
	CInteger(int=3);			// default 2 digits
